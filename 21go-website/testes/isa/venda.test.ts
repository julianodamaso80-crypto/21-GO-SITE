...
test('lista INTEIRA pelo codigo: 18 itens do VIP, o que nao entra, PDF e proximo passo', () => {
  ...
  assert.match(m, /tá tudo detalhado no seu PDF/)
  assert.ok(!/siga com a sua ativação|faz mais sentido pra você/.test(m), 'sem pergunta de fechamento')
  ...
  assert.ok(!/siga com a sua ativação/.test(mensagemBeneficios({ abertura: null, plano: compass.planos[0], pdfUrl: 'u', temOutros: false })))
...
test('quanto ele paga hoje vira fato calculado, com as diferencas (11/09/2026 21:13: "650" foi jogado fora)', () => {
  ...
  assert.match(c.linhas[0], /se ele perguntar se compensa/)
...
test('retomada pelo momento do cliente, sem repetir e sem nome (12/09/2026 13:43)', () => {
  ...
  assert.match(mensagemRetomada({ escolheuPlano: false, jaPerguntouProtecao: true, planoUnico: 'Veículos Especiais' }), /olhada na simulação\? o Veículos Especiais ficou como te mandei/)
  ...
  const r = mensagemRetomada({ escolheuPlano: false, jaPerguntouProtecao: true, planoUnico: null, planoSugerido: { nome: 'VIP', mensal: 227.41 } })
  assert.match(r, /o VIP ficou em R\$ 227,41\/mês/)
  assert.ok(!/faz sentido|compare|sigo com a ativação/.test(r), 'retomada sem empurrar')
